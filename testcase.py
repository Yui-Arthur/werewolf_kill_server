...
def run():
    ...
    #### 第一天發言 ####
    print_stage(nextStage(stub,room_name=room_name , stage_name="1-0-witch"))
    ...
    print_stage(nextStage(stub,room_name=room_name , stage_name="1-1-vote1"))

    #### !!!! Error 不應該進 第一天二次投票 ####
    #### !!!! Error 平票的人不能投票 ####
    #### 第一天二次投票 ####
    #### 0投6 ####
    print(sendUserOperation(stub , user=0 , operation="vote_or_not" , target=6 , chat="" , room_name=room_name , stage_name="1-1-vote2"))
    print_stage(nextStage(stub,room_name=room_name , stage_name="1-1-vote2"))
    ####################

    #### 第一天6獵人死掉發言及發動技能 ####
    #### 0,1,2,3,4,5 存活 ####
    #### !!!! Error 先發言就不能發動技能 反之亦然  ####
    ...
    #### 第二天公告誰死了 ####
    #### 1,2,3,5 存活 ####
    print_stage(nextStage(stub,room_name=room_name , stage_name="2-0-witch"))
    #### 第二天發言 ####
    print_stage(nextStage(stub,room_name=room_name , stage_name="2-1-check_end1"))
    for i in range(3):
